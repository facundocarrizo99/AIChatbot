...
from app.arca.electronic_invoice import ElectronicInvoice
from app.arca.journal import AFIPJournal

load_dotenv()

def test_authentication():
    print("Testing AFIP Authentication...")
    auth_service = AFIPAuthService()
    auth = auth_service.authenticate('wsfe')
    if auth:
        print("Authentication successful!")
        print(f"Token: {auth['token']}")
        print(f"Sign: {auth['sign']}")
        print(f"Expires: {auth['expiration']}")
    else:
        print("Authentication failed")


def test_cae_request():
    print("\nTesting CAE Request...")
    invoice_service = InvoiceService()

    # Example invoice data (adjust according to your needs)
    invoice_data = {
        'tipo_cbte': 1,  # Factura A
        'punto_vta': 1,  # Punto de venta
        'cbte_nro': 1,  # Número de comprobante
